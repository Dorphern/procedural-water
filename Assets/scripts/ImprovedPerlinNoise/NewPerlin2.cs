...
public class NewPerlin2 : MonoBehaviour
{
	...
	void Update () 
	{
		for (y = 0; y<height; y++)
		{
			for (x = 0; x<width; x++)
			{
				...
				{
					c += newNoise(x * noiseScale, y * noiseScale, z) * TWO_PI;
				}
				cols[x+y*width] = new Color(c,c,c,1);		
			}
		}
		z += Time.deltaTime * timeScale;
	...
	{
		float corners = (newNoise(x-1, y-1, z)+newNoise(x+1, y-1,z)+newNoise(x-1, y+1,z)+newNoise(x+1, y+1,z) ) / 16;
		float sides   = (newNoise(x-1, y,z)  +newNoise(x+1, y,z)  +newNoise(x, y-1,z)  +newNoise(x, y+1,z) ) /  8;
		float center  =  newNoise(x, y,z) / 4;
		return corners + sides + center;
	}
	
	float fade(float t) {
		return ((t*6 - 15)*t + 10)*Mathf.Pow(t,3);
	}
